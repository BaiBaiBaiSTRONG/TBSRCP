...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['SparseLoss', 'DenseLoss','ClsLoss'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            sparse_loss, dense_loss = base_model.module.get_loss(rt,gt)

            # print('sparse_loss')
            # print(sparse_loss.shape)
            ...
            cls_loss = F.binary_cross_entropy_with_logits(cls_pred, y_cls_true)
            # print("="*50)
            # print(cls_pred)
            # print(y_cls_true.shape)
            ...
            _loss = sparse_loss + cls_loss * cls_backward_weight
            _loss.backward()
            ...
            if args.distributed:
                ...
                losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, cls_loss * cls_lossRecord_weight])
            else:
                losses.update([sparse_loss.item() * 1000, dense_loss.item() * 1000, cls_loss* cls_lossRecord_weight])
        ...
        if train_writer is not None:
            ...
            train_writer.add_scalar('Loss/Epoch/Cls', losses.avg(2), epoch)
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    test_losses = AverageMeter(['SparseLossL1', 'SparseLossL2', 'DenseLossL1', 'DenseLossL2', 'ClsLoss'])
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            test_losses.update([sparse_loss_l1.item() * 1000, sparse_loss_l2.item() * 1000, dense_loss_l1.item() * 1000, dense_loss_l2.item() * 1000, cls_loss *cls_lossRecord_weight])
            ...
            _metrics = Metrics.get(dense_points, gt, cls_pred, y_cls_true)
    ...
    if val_writer is not None:
        ...
        val_writer.add_scalar('Loss/Epoch/Cls', test_losses.avg(2), epoch)
